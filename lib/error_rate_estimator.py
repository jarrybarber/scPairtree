...
from pairs_tensor_util import log_p_data_given_model_phis_and_errors, p_phi_given_model, _p_model_given_phi
...
def _calc_to_min_val(alphas,betas,phis,pairwise_occurances,d_rng_i):
    ...
    AB_scores     = np.array([[ np.log(_p_model_given_phi(Models.A_B,phis[i],phis[j]))
                                 ...
                                 + log_p_data_given_model_phis_and_errors(Models.A_B,pairwise_occurances[:,:,i,j],alphas[i],alphas[j],betas[i],betas[j],phis[i],phis[j],d_rng_i) 
    ...
    branch_scores = np.array([[np.log(_p_model_given_phi(Models.diff_branches,phis[i],phis[j]))
                                ...
                                + log_p_data_given_model_phis_and_errors(Models.diff_branches,pairwise_occurances[:,:,i,j],alphas[i],alphas[j],betas[i],betas[j],phis[i],phis[j],d_rng_i)
                                if i>=j else 0 
                               for j in range(n_mut)] 
                              for i in range(n_mut)])
...
def estimate_error_rates(data, d_rng_i=DataRangeIdx.ref_var_nodata, variable_ado=True):
    ...
    if variable_ado:
        to_min = lambda x: _calc_to_min_val([x[0]]*n_mut,x[1:n_mut+1],x[n_mut+1:],pairwise_occurances,d_rng_i)
        bounds=[(0.000001,0.1)] + [(0.001,0.9)]*n_mut + [(0.0,1.0)]*n_mut
        beta0s  = np.random.beta(30,200,n_mut)
    else:
        to_min = lambda x: _calc_to_min_val([x[0]]*n_mut, [x[1]]*n_mut, x[2:],    pairwise_occurances,d_rng_i)
