...
from pairs_tensor_util import log_model_posterior, p_phi_given_model, _p_model_given_phi
...
def _calc_to_min_val(alphas,betas,phis,pairwise_occurances,d_rng_i):
    ...
    AB_scores     = np.array([[ _p_model_given_phi(Models.A_B,phis[i],phis[j])
                                 ...
                                 + log_model_posterior(Models.A_B,pairwise_occurances[:,:,i,j],alphas[i],alphas[j],betas[i],betas[j],phis[i],phis[j],d_rng_i) 
    ...
    branch_scores = np.array([[_p_model_given_phi(Models.diff_branches,phis[i],phis[j])
                                ...
                                + log_model_posterior(Models.diff_branches,pairwise_occurances[:,:,i,j],alphas[i],alphas[j],betas[i],betas[j],phis[i],phis[j],d_rng_i)
                                if i>=j else 0 
                               for j in range(n_mut)] 
                              for i in range(n_mut)])
...
def estimate_error_rates(data, d_rng_i=DataRangeIdx.ref_var_nodata, variable_ado=True):
    ...
    if variable_ado:
        ...
        bounds=[(0.000001,0.1)] + [(0.001,0.5)] + [(0.0,1.0)]*n_mut
