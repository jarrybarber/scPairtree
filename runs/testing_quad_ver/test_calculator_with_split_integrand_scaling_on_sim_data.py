...
from score_calculator_quad_method_w_split_integrand_scaling import calc_ancestry_tensor


def plot_raw_scores(scores, outdir):
    
    range = [np.min(scores),np.max(scores)]
    if np.isneginf(range[0]):
        range[0] = -1000.0
    ...
    plt.figure()
    plt.subplot(231)
    plt.imshow(scores[0,:,:],vmin=range[0],vmax=range[1],cmap=cmap)
    plt.title("Model 1; A-->B")
    # plt.colorbar()
    plt.subplot(232)
    plt.imshow(scores[1,:,:],vmin=range[0],vmax=range[1],cmap=cmap)
    plt.title("Model 2; B-->A")
    # plt.colorbar()
    plt.subplot(233)
    plt.imshow(scores[2,:,:],vmin=range[0],vmax=range[1],cmap=cmap)
    plt.title("Model 3; Co-clustered")
    # plt.colorbar()
    plt.subplot(234)
    plt.imshow(scores[3,:,:],vmin=range[0],vmax=range[1],cmap=cmap)
    plt.title("Model 4; Cousins")
    # plt.colorbar()
    plt.subplot(235)
    plt.imshow(scores[4,:,:],vmin=range[0],vmax=range[1],cmap=cmap)
...
def calc_anc_tensor(data, alpha, beta):
...
def main():
    ...
    sims_to_do =["tree6_es1_phis1_nc3240"]
    for sim in sims_to_do:
        ...
        outdir = os.path.join(OUT_DIR, "testing_quad_ver", "split_integrand_scaling_model_on_simulated_data", sim)
        ...
        start = time.time()        
        scores = calc_anc_tensor(data, FP, FN)
        end = time.time()
