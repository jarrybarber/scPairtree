...
def plot_FP_and_FN_jointly(data, real_anc_mat, fns, fps, actual_fn, actual_fp, outdir):
    ...
    for i, alpha in enumerate(fps):
        for j, beta in enumerate(fns):
            print("(i,j) = (",i,",",j,")")
            scores = calc_ancestry_tensor(alpha, beta, n11, n10, n01, n00, verbose=False)
...
def main():
    ...
    for sim in sims:
        ...
        outdir = os.path.join(OUT_DIR,"testing_quad_ver","estimating_error_rates","grid_search", sim)
